...
const botSchema = new mongoose.Schema({
  ...
  created_at:    { type: Date, default: Date.now },
  // ── File backup in MongoDB ──────────────────────
  file_backup:   { type: Buffer, default: null },   // ZIP/file content
  file_backup_name: { type: String, default: '' },  // original filename
  file_backup_saved_at: { type: Date, default: null },
});
...
class Database {
  ...
  // ── Save file buffer to MongoDB ─────────────────
  async saveBotFile(uid, botName, fileBuffer, fileName) {
    if (!isMongoConnected) return false;
    try {
      await Bot.updateOne(
        { user_id: uid, bot_name: botName },
        { $set: {
          file_backup: fileBuffer,
          file_backup_name: fileName,
          file_backup_saved_at: new Date(),
        }}
      );
      return true;
    } catch (e) {
      logger.error(`saveBotFile error: ${e.message}`);
      return false;
    }
  }

  // ── Get saved file buffer from MongoDB ──────────
  async getBotFile(uid, botName) {
    if (!isMongoConnected) return null;
    try {
      const bot = await Bot.findOne(
        { user_id: uid, bot_name: botName },
        { file_backup: 1, file_backup_name: 1 }
      ).lean();
      if (!bot?.file_backup) return null;
      return { buffer: bot.file_backup, fileName: bot.file_backup_name };
    } catch (e) {
      logger.error(`getBotFile error: ${e.message}`);
      return null;
    }
  }
