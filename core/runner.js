...
export async function installDeps(dir, uid) {
  ...
  if (fs.existsSync(reqPath)) {
    // requirements.txt hai — seedha install karo
    ...
    try {
      ...
      if (uid) await _sendFn(uid, '✅ <b>Python packages installed!</b>', { parse_mode: 'HTML' });
    } catch (e) {
      if (uid) await _sendFn(uid, `⚠️ <b>pip install warning:</b>\n<code>${String(e.message).slice(-400)}</code>`, { parse_mode: 'HTML' });
    }
  } else {
    // requirements.txt nahi hai — auto-detect karo
    const hasPyFiles = fs.readdirSync(dir).some(f => f.endsWith('.py')) ||
      (function check(d, dep=0) {
        if (dep>2) return false;
        try { return fs.readdirSync(d).some(f => { const fp=path.join(d,f); return f.endsWith('.py') || (fs.statSync(fp).isDirectory() && check(fp,dep+1)); }); } catch { return false; }
      })(dir);
    ...
    if (hasPyFiles) {
      ...
      if (packages.length > 0) {
        ...
        try {
          ...
          if (uid) await _sendFn(uid, '✅ <b>Auto packages installed!</b>', { parse_mode: 'HTML' });
        } catch (e) {
          if (uid) await _sendFn(uid, `⚠️ <b>Auto install warning:</b>\n<code>${String(e.message).slice(-400)}</code>`, { parse_mode: 'HTML' });
        }
      }
    }
  }
